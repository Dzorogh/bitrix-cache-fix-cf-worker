...
async function handleRequest(request: Request) {
	...
	modifiedRequest = await addParamToSplitMobileAndDesktop(modifiedRequest)
...
async function addParamFromCookie(request: Request, cookieName: string, defaultValue = '') {
	let url = new URL(request.url)

	const cookieValue = getCookie(request, cookieName)

	url.searchParams.set('_CF_CACHE_' + cookieName, cookieValue ? cookieValue : defaultValue);

	return new Request(url, request)
}

async function addParamToSplitMobileAndDesktop(request: Request) {
	let url = new URL(request.url)

	url.searchParams.set('_CF_CACHE_MOBILE', isMobile(request.headers.get('User-Agent') ?? '') ? '1' : '0');
